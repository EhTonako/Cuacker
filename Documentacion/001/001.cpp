...
string decodificarNumero(int num, const string* codigos) {
    if (num < 1 || num > 30) return "";
    return codigos[num - 1];
}

int main() {
    const string decodificaciones[30] = {
        "Afirmativo.",
        "Negativo.",
    ...
    for (int i = 0; i < num_casos; i++) {
        ...
        if (numero >= 1 && numero <= 30) {
            cout << decodificarNumero(numero, decodificaciones) << endl;
        }
    }
