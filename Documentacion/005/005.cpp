...
static void imprimir_resultado_busqueda(const string& eco, const Cuac& actual) {
    cout << eco << '\n';
    cout << "1. ";
    actual.escribir();
    cout << "Total: 1 cuac\n";
...
int main() {
    ...
    while (cin >> comando) {
        if (comando == "exit") {
            ...
            int N;
            if (!(cin >> N)) break;
            if (hayActual) imprimir_resultado_busqueda("last " + to_string(N), actual);
        } else if (comando == "follow") {
            string nombre;
            if (!(cin >> nombre)) break;
            if (hayActual) imprimir_resultado_busqueda("follow " + nombre, actual);
        } else if (comando == "date") {
            Fecha fmin, fmax;
            if (!fmin.leerFecha()) break;
            if (!fmax.leerFecha()) break;
            if (hayActual) {
                cout << "date ";
                fmin.escribirFecha();
                cout << " ";
            ...
            string etiqueta;
            if (!(cin >> etiqueta)) break;
            if (hayActual) imprimir_resultado_busqueda("tag " + etiqueta, actual);
        } else {
            break;
        }
    }
